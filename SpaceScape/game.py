...
pygame.mixer.init()
game_sound = pygame.mixer.music.load('sonidos/space.mp3')
game_over_sound = pygame.mixer.Sound('sonidos/ouch-116112.mp3')  
victoria_sonido = pygame.mixer.Sound('sonidos/victory.mp3')
...
def mostrar_mensaje(texto, color, tamano, y_offset=0):
    fuente = pygame.font.Font('freesansbold.ttf', tamano)
    mensaje = fuente.render(texto, True, color)
    mensaje_rect = mensaje.get_rect(center=(SCREEN_WIDTH // 2, (SCREEN_HEIGHT // 2) + y_offset))
...
def pantalla_fin_juego():
    ...
    mostrar_mensaje("Presiona 'R' para reiniciar o 'Q' para salir.", BLANCO, 36, y_offset=100)  # Ajusta el y_offset a 100 o el valor deseado
    pygame.display.update()
...
def pantalla_victoria():
    ...
    mostrar_mensaje("¡Victoria, Has completado el juego!", VERDE, 72)
    pygame.display.update()
...
class Game:
    ...
    def initialize(self):
        ...
        self.player.rect.bottom = SCREEN_HEIGHT - 10  # Centrado en el suelo, ajusta el valor para estar justo encima del borde
        self.movement = 0
    ...
    def update(self, delta_time):
        ...
        if self.lost or not self.started or self.victory:
            for event in events:
    ...
    def render(self):
        ...
        text_score = self.font.render('Puntuación: ' + str(display_score), True, (255, 255, 255))
